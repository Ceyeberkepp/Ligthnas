...
export const catalog = Object.freeze([
  ...
  { id: 'influxdb', name: 'InfluxDB', category: 'Database', image: 'influxdb:2', port: 8186, containerPort: 8086, memory: '1g', description: 'Time-series database for monitoring, sensors and automation.', source: 'InfluxData', volumes: [['data', '/var/lib/influxdb2']] },
  ...
  { id: 'trilium-next', name: 'TriliumNext Notes', category: 'Productivity', image: 'triliumnext/notes:latest', port: 8110, containerPort: 8080, memory: '1g', description: 'Hierarchical personal knowledge base and note-taking system.', source: 'TriliumNext', volumes: [['data', '/home/node/trilium-data']] },
  ...
  // Expanded community catalog. These are single-container applications that
  // fit LightNAS's one-click runtime model without pretending to be a direct
  // TrueNAS catalog import.
  { id: 'homepage', name: 'Homepage', category: 'Dashboard', image: 'ghcr.io/gethomepage/homepage:latest', port: 8301, containerPort: 3000, memory: '512m', description: 'Modern application dashboard for homelabs and self-hosted services.', source: 'Homepage', volumes: [['config', '/app/config']] },
  { id: 'homer', name: 'Homer', category: 'Dashboard', image: 'b4bz/homer:latest', port: 8302, containerPort: 8080, memory: '256m', description: 'Simple static dashboard for organizing self-hosted services.', source: 'Homer', volumes: [['assets', '/www/assets']] },
  { id: 'flame', name: 'Flame', category: 'Dashboard', image: 'pawelmalak/flame:latest', port: 8303, containerPort: 5005, memory: '512m', description: 'Self-hosted start page with application and bookmark management.', source: 'Flame', volumes: [['data', '/app/data']] },
  { id: 'nocodb', name: 'NocoDB', category: 'Productivity', image: 'nocodb/nocodb:latest', port: 8304, containerPort: 8080, memory: '1g', description: 'Spreadsheet-style interface for databases with built-in local storage support.', source: 'NocoDB', volumes: [['data', '/usr/app/data']] },
  { id: 'baserow', name: 'Baserow', category: 'Productivity', image: 'baserow/baserow:latest', port: 8305, containerPort: 80, memory: '2g', description: 'Open-source no-code database and collaboration workspace.', source: 'Baserow', volumes: [['data', '/baserow/data']] },
  { id: 'metabase', name: 'Metabase', category: 'Analytics', image: 'metabase/metabase:latest', port: 8306, containerPort: 3000, memory: '2g', description: 'Business intelligence dashboards and data exploration.', source: 'Metabase', volumes: [['data', '/metabase-data']] },
  { id: 'excalidraw', name: 'Excalidraw', category: 'Productivity', image: 'excalidraw/excalidraw:latest', port: 8307, containerPort: 80, memory: '512m', description: 'Collaborative whiteboard and diagramming interface.', source: 'Excalidraw', volumes: [] },
  { id: 'libretranslate', name: 'LibreTranslate', category: 'AI', image: 'libretranslate/libretranslate:latest', port: 8308, containerPort: 5000, memory: '2g', description: 'Self-hosted machine translation service and web interface.', source: 'LibreTranslate', volumes: [['data', '/home/libretranslate/.local']] },
  { id: 'docuseal', name: 'DocuSeal', category: 'Productivity', image: 'docuseal/docuseal:latest', port: 8309, containerPort: 3000, memory: '1g', description: 'Open-source document signing and form workflow application.', source: 'DocuSeal', volumes: [['data', '/data']] },
  { id: 'silverbullet', name: 'SilverBullet', category: 'Productivity', image: 'silverbulletmd/silverbullet:latest', port: 8310, containerPort: 3000, memory: '512m', description: 'Markdown knowledge base and programmable personal workspace.', source: 'SilverBullet', volumes: [['space', '/space']] },
  { id: 'ntfy', name: 'ntfy', category: 'Notifications', image: 'binwiederhier/ntfy:latest', port: 8311, containerPort: 80, memory: '256m', description: 'Simple self-hosted push notification service.', source: 'ntfy', volumes: [['cache', '/var/cache/ntfy'], ['config', '/etc/ntfy']], command: ['serve'] },
  { id: 'microbin', name: 'MicroBin', category: 'Utility', image: 'danielszabo99/microbin:latest', port: 8312, containerPort: 8080, memory: '256m', description: 'Small self-hosted pastebin and file/text sharing service.', source: 'MicroBin', volumes: [['data', '/app/pasta_data']] },
  { id: 'pingvin-share', name: 'Pingvin Share', category: 'Files', image: 'stonith404/pingvin-share:latest', port: 8313, containerPort: 3000, memory: '768m', description: 'Self-hosted file sharing platform with expiring links.', source: 'Pingvin Share', volumes: [['data', '/opt/app/backend/data'], ['images', '/opt/app/frontend/public/img']] },
  { id: 'gokapi', name: 'Gokapi', category: 'Files', image: 'f0rc3/gokapi:latest', port: 8314, containerPort: 53842, memory: '512m', description: 'Lightweight file sharing server with expiring downloads.', source: 'Gokapi', volumes: [['data', '/app/data'], ['config', '/app/config']] },
  { id: 'sftpgo', name: 'SFTPGo', category: 'Files', image: 'drakkan/sftpgo:latest', port: 8315, containerPort: 8080, memory: '1g', description: 'Managed SFTP, HTTP and WebDAV file transfer server with web administration.', source: 'SFTPGo', volumes: [['data', '/srv/sftpgo'], ['home', '/var/lib/sftpgo']], extraPorts: [[2022, 2022, 'tcp']] },
  { id: '2fauth', name: '2FAuth', category: 'Security', image: '2fauth/2fauth:latest', port: 8316, containerPort: 8000, memory: '512m', description: 'Self-hosted two-factor authentication code manager.', source: '2FAuth', volumes: [['data', '/2fauth']] },
  { id: 'wallos', name: 'Wallos', category: 'Productivity', image: 'bellamy/wallos:latest', port: 8317, containerPort: 80, memory: '512m', description: 'Subscription tracking and personal recurring-expense dashboard.', source: 'Wallos', volumes: [['db', '/var/www/html/db'], ['logos', '/var/www/html/images/uploads/logos']] },
  { id: 'grocy', name: 'Grocy', category: 'Home', image: 'lscr.io/linuxserver/grocy:latest', port: 8318, containerPort: 80, memory: '768m', description: 'Household inventory, shopping, chores and meal planning.', source: 'LinuxServer.io', volumes: [['config', '/config']] },
  { id: 'kavita', name: 'Kavita', category: 'Media', image: 'jvmilazz0/kavita:latest', port: 8319, containerPort: 5000, memory: '1g', description: 'Digital library for comics, manga and ebooks.', source: 'Kavita', volumes: [['config', '/kavita/config'], ['@files', '/data:ro']] },
  { id: 'airsonic-advanced', name: 'Airsonic Advanced', category: 'Media', image: 'lscr.io/linuxserver/airsonic-advanced:latest', port: 8320, containerPort: 4040, memory: '1g', description: 'Subsonic-compatible music streaming server.', source: 'LinuxServer.io', volumes: [['config', '/config'], ['@files', '/music:ro']] },
  { id: 'netdata', name: 'Netdata', category: 'Monitoring', image: 'netdata/netdata:stable', port: 8321, containerPort: 19999, memory: '1g', description: 'Real-time infrastructure monitoring dashboard.', source: 'Netdata', volumes: [['config', '/etc/netdata'], ['lib', '/var/lib/netdata'], ['cache', '/var/cache/netdata']] },
  { id: 'prometheus', name: 'Prometheus', category: 'Monitoring', image: 'prom/prometheus:latest', port: 8322, containerPort: 9090, memory: '1g', description: 'Metrics collection, querying and alerting engine.', source: 'Prometheus', volumes: [['data', '/prometheus']] },
  { id: 'node-exporter', name: 'Node Exporter', category: 'Monitoring', image: 'prom/node-exporter:latest', port: 8323, containerPort: 9100, memory: '256m', description: 'Prometheus-compatible host metrics exporter.', source: 'Prometheus', volumes: [] },
  { id: 'rabbitmq', name: 'RabbitMQ Management', category: 'Database', image: 'rabbitmq:3-management', port: 8324, containerPort: 15672, memory: '1g', description: 'Message broker with browser-based management console.', source: 'RabbitMQ', volumes: [['data', '/var/lib/rabbitmq']], extraPorts: [[5672, 5672, 'tcp']] },
  { id: 'adminer', name: 'Adminer', category: 'Database', image: 'adminer:latest', port: 8325, containerPort: 8080, memory: '256m', description: 'Compact browser database administration tool.', source: 'Docker Official Image', volumes: [] },
  { id: 'cloudbeaver', name: 'CloudBeaver', category: 'Database', image: 'dbeaver/cloudbeaver:latest', port: 8326, containerPort: 8978, memory: '1g', description: 'Browser-based database administration from the DBeaver project.', source: 'DBeaver', volumes: [['workspace', '/opt/cloudbeaver/workspace']] },
  { id: 'cyberchef', name: 'CyberChef', category: 'Security', image: 'mpepping/cyberchef:latest', port: 8327, containerPort: 8000, memory: '512m', description: 'Browser toolkit for encoding, decoding, crypto and data transformation.', source: 'CyberChef', volumes: [] },
  { id: 'webtop', name: 'Webtop', category: 'Utility', image: 'lscr.io/linuxserver/webtop:ubuntu-kde', port: 8328, containerPort: 3000, memory: '2g', description: 'Full Linux desktop environment accessible in a browser.', source: 'LinuxServer.io', volumes: [['config', '/config']] },
  { id: 'ollama', name: 'Ollama', category: 'AI', image: 'ollama/ollama:latest', port: 8329, containerPort: 11434, memory: '4g', description: 'Local model runtime API for supported AI models.', source: 'Ollama', volumes: [['models', '/root/.ollama']] },
  { id: 'flowise', name: 'Flowise', category: 'AI', image: 'flowiseai/flowise:latest', port: 8330, containerPort: 3000, memory: '2g', description: 'Visual builder for LLM workflows, tools and agents.', source: 'Flowise', volumes: [['data', '/root/.flowise']] },
  { id: 'localai', name: 'LocalAI', category: 'AI', image: 'localai/localai:latest', port: 8331, containerPort: 8080, memory: '4g', description: 'OpenAI-compatible local inference API with broad model support.', source: 'LocalAI', volumes: [['models', '/models']] },
  { id: 'homebridge', name: 'Homebridge', category: 'Home', image: 'homebridge/homebridge:latest', port: 8332, containerPort: 8581, memory: '1g', description: 'HomeKit bridge for smart-home devices and plugins.', source: 'Homebridge', volumes: [['data', '/homebridge']] }
]);
